...
def meta_gradient_step(model: Module,
    ...
    for meta_batch in x:
        ...
        for inner_batch in range(inner_train_steps):
            ...
            gradients = torch.autograd.grad(
                loss, fast_weights.values(), create_graph=create_graph)
    ...
    if order == 1:
        if train:
            ...
            logits = model(torch.zeros(
                (k_way, ) + data_shape).to(device, dtype=torch.double))
            loss = loss_fn(
                logits, create_nshot_task_label(k_way, 1).to(device))
            loss.backward()
            optimiser.step()
